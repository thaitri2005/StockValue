...
describe('Server Basic Functionality', () => {
    const clientPath = process.env.CLIENT_DIR || path.join(__dirname, '..', 'client');
    ...
    test('should return historical data for a given symbol', async () => {
        ...
        const response = await request(server).get('/api/historical/AAPL?from=2023-01-01&to=2023-01-31');
        expect(response.status).toBe(200);
